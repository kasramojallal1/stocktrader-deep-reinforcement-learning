...
class StockEnvTrade(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            plt.savefig(
                'trader/drl_stock_trader/results/account_value_trade_{}_{}.png'.format(self.model_name, self.iteration))
            plt.close()

            df_total_value = pd.DataFrame(self.asset_memory)
            df_total_value.to_csv(
                'trader/drl_stock_trader/results/account_value_trade_{}_{}.csv'.format(self.model_name, self.iteration))
            ...
            self.socket.send(text_data=json.dumps({
                'type': 'terminal',
                'message': f'total_reward:{self.state[0] + sum(np.array(self.state[1:(STOCK_DIM + 1)]) * np.array(self.state[(STOCK_DIM + 1):(STOCK_DIM * 2 + 1)])) - self.asset_memory[0]:.2f}'
            }))
            self.socket.send(text_data=json.dumps({
                'type': 'terminal',
            ...
            df_rewards.to_csv('trader/drl_stock_trader/results/account_rewards_trade_{}_{}.csv'.format(self.model_name,
                                                                                                       self.iteration))
